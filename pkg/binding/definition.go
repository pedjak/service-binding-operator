...
func (d *stringFromDataFieldDefinition) Apply(u *unstructured.Unstructured) (Value, error) {
	...
	res, err := jsonpath.GetValuesByJSONPath(u.Object, d.path)
	if err != nil {
		return nil, err
	}
...
func collectSourceValuesWithKey(i interface{}, sourceValue string, sourceKey string) map[interface{}]interface{} {
	res := make(map[interface{}]interface{})
	switch v := i.(type) {
	case map[string]interface{}:
		key := v[sourceKey]
		res[key] = v[sourceValue]
	case []interface{}:
		for _, item := range v {
