...
func (r *ServiceBinding) ValidateUpdate(old runtime.Object) error {
	...
	status := r.Status.Conditions
	deletiontimestamp := r.ObjectMeta.GetDeletionTimestamp()
	for _, condition := range status {
		if (condition.Type == "Ready") && (condition.Status == metav1.ConditionTrue) && deletiontimestamp == nil {
			err := errors.New("Cannot Update Service Binding Resource if Ready Status is True")
			return err
		}
	}
